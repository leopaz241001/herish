...
const createReminder = function () {
  ...
  formReminder.addEventListener('submit', async function(e) {
    ...
    const url = idReminder ? `https://herish.id.vn/api/special-days/${idReminder}` : 'https://herish.id.vn/api/special-days';
...
async function handleReminderAction() {
  $(document).on("click", ".btn-open-popup", async function () {
    ...
    if(event_id && ariaLabel === "Chỉnh sửa lời nhắc") {
      ...
      try {
        let url = `https://herish.id.vn/api/special-days/${event_id}`;
        const access_token = localStorage.getItem('access_token');
        const res = await fetch(url, {
          method: 'GET',
          headers: {
...
async function deleteReminder() {
  ...
  $(".btn-delete-reminders").on("click", async function () {
    try {
      const access_token = localStorage.getItem('access_token');
      const res = await fetch(`https://herish.id.vn/api/special-days/${event_id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${access_token}`
